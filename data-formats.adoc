...
ファイルが所定のフォルダに格納されると、インテグレーションが実行されて、コンソールに ファイルの本文と、`Unmarshal` で変換した後の Log が表示され、`data/output` にファイルが格納されます。（格納したファイルは、デフォルトの設定では `data/input` 配下に `.camel` フォルダが作成され、そちらに移動します）

CSV Data Format で Unmarshal したデータは、JavaのList型になります。

image::data-formats/dataformats-0013.png[] 
